 ...
 *
 * CAMBIO: guardarPDF() ahora llama a PDFUtils.enviarAlScript()
...
function _buildPDFHTML() {
  ...
  CRITERIOS.forEach(function (sec) {
    filas +=
      '<tr><td colspan="6" style="background:linear-gradient(135deg,#1e40af,#3b82f6);color:white;font-weight:800;padding:7px 12px;font-size:11px;">' +
      sec.seccion +
      '</td></tr>';
    sec.items.forEach(function (item) {
      var celdas = '';
  ...
  return (
    ...
    '.ft{background:#1e40af;color:white;font-weight:800;padding:5px 10px;font-size:10px;}' +
    ...
    '.fbox{text-align:center;width:210px;}.fesp{height:65px;border-bottom:1.5px dashed #aaa;}' +
    '.fnom{font-weight:800;font-size:11px;color:#1e3a5f;margin-top:6px;}.fcar{font-size:9.5px;color:#5a7a9a;font-weight:600;}' +
    '</style></head><body>' +
    '<div class="hdr"><div><div class="tit">RÚBRICA DE EVALUACIÓN DOCENTE</div>' +
...
function guardarPDF() {
  ...
  PDFUtils.enviarAlScript(
    ...
    'evaluacion',
    function (urlPDF) {
      // El PDF quedó guardado en Classroom — se puede abrir con la URL
  ...
  // Imprimir como siempre (sin esperar la respuesta del script)
